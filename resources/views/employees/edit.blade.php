...
@section('page-title', 'Mitarbeiter bearbeiten')

@section('content')
<div class="max-w-3xl">
    <div class="bg-white rounded-xl shadow-sm p-6">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-lg font-semibold text-gray-800">{{ $employee->full_name }} bearbeiten</h2>
            <a href="/employees/{{ $employee->id }}" class="text-sm text-gray-500 hover:text-gray-700">Abbrechen</a>
        </div>

        <form method="POST" action="/employees/{{ $employee->id }}" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea id="address" name="address" rows="2"
                                  class="w-full px-4 py-2.5 border {{ $errors->has('address') ? 'border-red-400' : 'border-gray-300' }} rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('address', $employee->address) }}</textarea>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="kita_id" name="kita_id" required
                            ...
                            <option value="{{ $kita->id }}" {{ old('kita_id', $employee->kita_id) == $kita->id ? 'selected' : '' }}>{{ $kita->name }}</option>
                            @endforeach
                        </select>
                        @else
                        <input type="hidden" name="kita_id" value="{{ $employee->kita_id }}">
                    ...
                    <div>
                        ...
                        <input type="date" id="start_date" name="start_date" value="{{ old('start_date', $employee->start_date?->format('Y-m-d')) }}" required
                    ...
                    <div>
                        ...
                        <input type="date" id="end_date" name="end_date" value="{{ old('end_date', $employee->end_date?->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <input type="number" id="weekly_hours" name="weekly_hours" value="{{ old('weekly_hours', $employee->weekly_hours) }}" step="0.5" min="0" max="80" required
            ...
            <div>
                ...
                <textarea id="notes" name="notes" rows="3"
                          class="w-full px-4 py-2.5 border {{ $errors->has('notes') ? 'border-red-400' : 'border-gray-300' }} rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('notes', $employee->notes) }}</textarea>
